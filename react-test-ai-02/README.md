...
`totalItems`가 추정치라서 **이 API로 정확한 전체 페이지 수 표시는 원리적으로 불가능**하다.
...
### 정리
02는 fetch · useEffect · 상태 처리 **문법을 익히는 데** 초점이 맞춰져 있고
(`App_useEffect.jsx`를 따로 남겨둔 것도 그 목적),
ai-02는 같은 기능을 **엣지 케이스와 관심사 분리** 쪽으로 다듬은 버전이다.
